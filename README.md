...
- **get_operator_routes の0駅バグ修正・検索結果表示の改善（v2.52.0）**
  - `get_operator_routes` で ODPT が駅データ（stationOrder）を返さない愛称路線（東武スカイツリーライン・東武アーバンパークライン・押上-曳舟ブランチ）が0駅で表示されるバグを修正。`ODPT_RAILWAY_NAME_MAP`（路線ID→内部グラフ路線名）で解決し内蔵グラフの駅を補完（スカイツリーライン30駅・アーバンパークライン35駅）。「スカイツリーライン」「アーバンパークライン」など東武なし表記でも事業者を逆引き解決
  - `search_route` の表示順を「実ルート→AIインテリジェントアドバイス→その他」に変更し、JSON側も `ai_transit_advice` を実ルート直後に移動。`jsonResponse` に `displayText` オプションを追加（#122: ホストLLMの要約による安全情報喪失を防止）
