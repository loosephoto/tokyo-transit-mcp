...
- **イシュー#76 将来課題の整理＋公的機関の検索案内を駅情報・バス検索に展開（v2.38.9）**
  - **#76-1 特急・新幹線の経路検索グラフ対応は実装しない**: 運賃体系（特急券・指定席券）と停車駅パターンが一般路線と異なるため検索グラフには組み込まず、既存の駅案内（みどりの窓口・指定席券売機）表示で対応する方針をコードコメント・案内メッセージ（ja/en/zh）に明記
  - **#76-3 東海汽船GTFSの復旧監視を強化**: 実GTFS取得失敗時は「ハードコード19港フォールバック使用中」を警告ログで明示し、取得成功時は「real GTFS recovered」で復旧を検知。1時間TTLのキャッシュ更新時に自動再試行されるため、復旧後は次回ロードで実データに自動復帰（確認手順はREADME/SKILL.mdに明文化）
  - **#76-4 一時ファイルの恒久対策を確認**: `.gitignore` の `_tmp*` / `scripts/_tmp-*` による誤コミット防止（既対応）を確認し、ワークツリーの残骸を整理
  - **公的機関の検索案内を駅情報・バス検索にも展開**: `get_station_info`（駅検索）と `search_bus`（曖昧時・未検出時・成功時）に `gov_facility_search_support` を追加し、`search_route` と同様に「その場所周辺の役所・出張所・公民館」をワンクリックで検索可能に
  - **検証** — build PASS・probe-all-lang 26/26 PASS・フェリー/特急系ツールの動作確認済み
...
- **整理 issue #76 的未来课题＋将公家机关搜索指南扩展到车站信息・巴士搜索（v2.38.9）**
  - **#76-1 不实现特急・新干线的路线搜索图支持**：由于票价体系（特急券・指定席券）与停靠站模式不同于普通线路，不将其纳入搜索图，继续通过现有的车站指南（绿色窗口・指定席售票机）显示对应，并在代码注释・提示消息（ja/en/zh）中明确该方针
  - **#76-3 加强东海汽船 GTFS 的恢复监控**：实 GTFS 获取失败时以警告日志明确「使用内置19港备用数据」，成功获取时以「real GTFS recovered」检测恢复。由于每次 1 小时 TTL 缓存刷新时自动重试，恢复后下次加载即自动切回实数据（确认步骤已写入 README/SKILL.md）
  - **#76-4 确认临时文件的永久对策**：确认 `.gitignore` 的 `_tmp*` / `scripts/_tmp-*` 可防止误提交（已对应），并清理工作区的残留文件
  - **将公家机关搜索指南扩展到车站信息・巴士搜索**：在 `get_station_info`（车站搜索）与 `search_bus`（模糊・未找到・成功时）中新增 `gov_facility_search_support`，与 `search_route` 一样可一键搜索「该地点附近的政府机关・办事处・公民馆」
  - **验证** — build 通过・probe-all-lang 26/26 通过・渡轮/特急类工具动作确认完毕
