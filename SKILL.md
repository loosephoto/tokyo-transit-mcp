...
### v2.28.0（2026-08-08）— 未対応イシュー一括解決（#30・#31・#33・#36・#37・#39・#43）

- **高崎線・宇都宮線に上野〜大宮間を追加**: 上野・尾久・赤羽・浦和・さいたま新都心（上野→高崎 0乗換58分・上野→宇都宮 0乗換55分）。新規駅: 尾久。
- **JR総武線快速・成田線を追加**: 東京〜佐倉（快速）＋佐倉〜成田〜成田空港。東京→成田空港 1乗換49分（従来は京成経由127分）。新規駅: 新日本橋・馬喰町・東千葉・都賀・四街道・物井・佐倉・酒々井・成田。
- **武蔵野線に大崎支線を追加**: `['大崎','西浦和']`（むさしの号ルート・大崎→府中本町 1乗換29分）。
- **東武越生線を追加**: 坂戸〜越生（7駅・東上線と接続）。
- **埼玉新都市交通（ニューシャトル）を追加**: 大宮〜内宿（13駅）。鉄道博物館ランドマークの最寄りを「鉄道博物館（大成）」に更新（徒歩1分）。
  - 🔴 **注意**: 2024年4月に大成駅が「鉄道博物館（大成）」に改称済み。古い「大成」表記で解決しないこと。
- **千葉都市モノレール1号線・江ノ島電鉄を追加**: 千葉みなと〜千城台（18駅）・藤沢〜鎌倉（15駅）。都賀（総武本線）・藤沢（JR/小田急）・鎌倉（JR横須賀線）で接続。
- **山手線の環状表現を CIRCULAR_LINES 方式に統一**: 末尾の重複「東京」を削除し30駅の公式駅順に一致（check-railway-integrity の期待値と一致）。
- **ダイクストラのコスト関数を改善（#37）**:
  - 従来: transfers（乗換回数）辞書順優先 → 0乗換の遠回りが1乗換の直通に常に勝つ問題
  - 新方式: `cost = dist + transfers × TRANSFER_PENALTY`（TRANSFER_PENALTY を3→10に増強）＋ 同コスト時は乗換数の少ない方を優先（`betterThan`）
  - 改善例: 青砥→成田空港 85分→24分（スカイアクセス）/ 日暮里→成田空港 103分→42分 / 大宮→船橋 野田線直通0乗換 / 五反田→蒲田 池上線直通0乗換
  - 🔴 **注意（再発バグクラス）**: ダイクストラの打ち切り条件は「pop したノードが bestGoal より良くない（cost大 or 同costで乗換多）なら break」にする。単純な cost 比較だと同コストの別経路（乗換数の異なる）が探索されない。best 更新も `betterThan` で一貫させること。
- **検証**: probe-all-lang(26/26)・test:walk・test-issues・test-transfer-pairs・check-railway-integrity 全PASS。

### v2.27.0（2026-08-08）— 東京圏の路線・ランドマーク拡充（Issue #29・#32・#38・#40・#41・#43・#44）

- **JR東海道線に各駅停車の駅を追加**: 辻堂・大磯・二宮・国府津・鴨宮・早川・根府川・真鶴・湯河原（小田原〜熱海間の駅欠落を解消）。
- **JR鶴見線に支線を追加**: 海芝浦支線（浅野〜新芝浦〜海芝浦）・大川支線（安善〜大川）を別路線として定義（分岐駅で本線と接続）。
- **西武の支線を追加**: 秩父線（飯能〜正丸〜芦ヶ久保〜西武秩父）・狭山線（西所沢〜上山口〜西武球場前）・有楽町線（練馬〜新桜台〜小竹向原）。
  - 🔴 **注意**: STATION_NAME_MAP で「有楽町線」エイリアスは既存の東京メトロ有楽町線（'yurakucho'）が先に定義済み。西武有楽町線は「西武有楽町線」表記のみで解決する（後勝ちで既存エイリアスを壊さないこと）。
- **東急新横浜線を追加**: 日吉⇔新横浜（2023年開業・相鉄新横浜線と直通）。日吉（東横線・目黒線）・新横浜（相鉄新横浜線・JR横浜線）で接続。
- **ランドマーク17施設を追加**: 鉄道博物館（大宮・徒歩20分）・地下鉄博物館（葛西）・東武博物館（東向島）・京王れいんらんど（多摩動物公園）・青梅鉄道公園（青梅）・電車とバスの博物館（向ヶ丘遊園）・横浜市電保存館（山手）・科学技術館（竹橋）・東京都水の科学館（豊洲）・東京都美術館（上野）・飛鳥山公園（王子）・東京都現代美術館（清澄白河）・サントリーホール（溜池山王）・みなとみらい21（みなとみらい）・すみだ水族館（とうきょうスカイツリー）・キッザニア東京（豊洲）・アクアシティお台場（台場）。
  - 🔴 **注意**: 実在駅名をランドマークに奪わせない（「お台場」は駅名ではなく「台場」が駅。LANDMARK_DEFS の station は実在駅名を指定すること）。
