...
### v2.42.1（2026-08-14）— 交通API障害・曖昧検索・キャッシュ処理を改善

- **時刻表API障害の誤判定を修正**: ODPT路線一覧取得失敗を `NO_DATA` として扱わず、サーキットブレーカーへ通知してAPIエラーとして返す
- **バス停の曖昧検索を停止**: 複数事業者・地域の候補が一致した場合、先頭候補を自動採用せず `AMBIGUOUS_BUS_STOP` と候補一覧を返す
- **キャッシュを真のLRU化**: 参照されたエントリを末尾へ移動し、容量超過時に最も長く未使用のエントリを削除
- **APIエラー分類を細分化**: 401/403認証エラー、5xxサーバーエラー、タイムアウト、内部エラーを構造化レスポンスで区別
- **検証**: 全対象モジュール `node --check`、修正回帰テスト、実データスモークテスト、`git diff --check` PASS
