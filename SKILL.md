...
### v2.38.3（2026-08-10）— 地下連絡通路で直結する近接異名駅6組を追加（Issue #69・都庁前⇔西新宿）

- `WALK_TRANSFERS` に6組追加（実在性は各事業者公式案内で裏取り済み）:
...
- 効果: 都庁前→西新宿 が **12分・乗換1回（新宿経由）→ 徒歩連絡8分・乗換0回** に短縮。本郷三丁目→仲御徒町も徒歩連絡経由10分に改善
- **検証**: `npm run build` 成功・`probe-all-lang` 26/26・`check-railway-integrity` PASS・`test-transfer-pairs` 33ペア全PASS・バウンス/連鎖なし
