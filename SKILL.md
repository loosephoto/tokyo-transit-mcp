...
### v2.42.2（2026-08-15）— 水上交通フェイルセーフ・公式リンク・天気地域表示を改善

- **水上交通の安全情報障害をフェイルセーフ化**: 津波・港湾気象APIの取得不能時は `MARITIME_SAFETY_UNKNOWN` を返し、通常航路を停止
- **安全判定状態を明示**: `active: true`（危険あり）、`active: false`（正常取得・危険なし）、`available: false`（判定不能）を区別し、多言語案内・公式確認先を返す
- **コミュニティバス公式リンクを更新**: 荒川、昭島、稲城、国立、新宿、東大和、文京、調布の現行自治体公式ページへ更新。運行終了した新宿WEバスは終了案内へ変更
- **天気地域表示を修正**: 横浜を日本語 `横浜`、英語 `Yokohama`、中国語 `横滨` として表示
- **検証**: 天気地域・荒天・津波・安全情報障害・公式リンク回帰テスト、`npm run build`、`git diff --check` PASS
